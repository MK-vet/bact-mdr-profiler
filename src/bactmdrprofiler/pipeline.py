...
from .mdr.classification import (
    build_class_matrix, identify_mdr, mdr_spectrum, prevalence_table, mdr_probability,
)
from .causal.discovery import pc_skeleton, motif_census, build_hybrid_network
from .network.hypergraph import (
    extract_hyperedges, hypergraph_centrality, interaction_information,
)
from .mdr.decision_v3 import next_best_test_evpi, pattern_mdl_compression, shapley_pattern_contributions
...
def _save(df, p):
    ...
    out = df.copy(); out[num] = out[num].round(4)
    out.to_csv(p, index=False); logger.info("  → %s (%d rows)", p, len(out))
...
class Pipeline:
    ...
    def _write_manifest(self, extra_inputs: list[str]) -> None:
        import platform
        import sys
        inp = [{"name": "phenotype", "path": self.cfg.input_csv, "sha256": sha256_file(self.cfg.input_csv)}]
        if self.cfg.gene_csv:
            inp.append({"name": "gene_layer", "path": self.cfg.gene_csv, "sha256": sha256_file(self.cfg.gene_csv)})
    ...
    def run(self) -> Dict[str, pd.DataFrame]:
        ...
        matched = {cls: [d for d in (drugs or []) if d in raw_cols] for cls, drugs in declared.items()}
        unmatched = {cls: [d for d in (drugs or []) if d not in raw_cols] for cls, drugs in declared.items()}
        ...
        (self.out / 'ontology_mapping_report.json').write_text(json.dumps(coverage_rep, indent=2, ensure_ascii=False))
        ...
        if map_rows:
            pd.DataFrame(map_rows).to_csv(self.out / 'ontology_mapping_report.csv', index=False)
        if class_df.shape[1] == 0 or n_classes_matched < int(getattr(c, 'min_classes_matched', 1)):
            raise ValueError(
        ...
        if n_classes_matched < int(c.ontology.mdr_threshold):
            logger.warning('Only %d antibiotic classes matched (threshold=%d) — MDR calls may be impossible or rare.', n_classes_matched, int(c.ontology.mdr_threshold))

        spectrum = mdr_spectrum(class_df, c.ontology.mdr_threshold)
        spectrum.insert(0, c.id_column, class_df.index.values)
        _save(spectrum, str(self.out / "mdr_spectrum.csv")); res["spectrum"] = spectrum

        prev = prevalence_table(class_df, c.confidence)
        _save(prev, str(self.out / "prevalence.csv")); res["prevalence"] = prev

        if c.probabilistic.enabled:
            pm = mdr_probability(class_df, c.ontology.mdr_threshold)
            _save(pm, str(self.out / "mdr_probability.csv")); res["p_mdr"] = pm

        # v3 decision-theoretic add-on: next best test (EVPI-like)
        try:
            evpi_costs = getattr(c, 'test_costs', None) if hasattr(c, 'test_costs') else None
            evpi = next_best_test_evpi(class_df, c.ontology.mdr_threshold, test_costs=evpi_costs)
            if not evpi.empty:
                _save(evpi, str(self.out / 'next_best_test_evpi.csv')); res['evpi']=evpi
        ...
        n_mdr = int((mdr_mask == True).sum())
        n_unc = int(mdr_mask.isna().sum())
        rate = (n_mdr / n_def * 100) if n_def else float("nan")
        logger.info("  MDR definite rate: %d/%d (%.1f%%), uncertain: %d", n_mdr, n_def, rate, n_unc)
        ...
        qc_rows = []
        if c.gene_csv:
            lr = load_layer_csv("AMR_genes", c.gene_csv, id_col=c.id_column, all_ids=class_df.index.tolist(),
                                feature_col=c.gene_layer.feature_column, value_col=c.gene_layer.value_column)
        ...
        if cov_rows:
            ...
            _save(cov_df, str(self.out / "layer_coverage.csv")); res["coverage"] = cov_df
        if qc_rows:
            qc_df = pd.concat(qc_rows, ignore_index=True)
            _save(qc_df, str(self.out / "feature_qc.csv")); res["qc"] = qc_df

        G, edge_df = build_hybrid_network(class_df, gene_df,
                                           c.network.alpha, c.network.fdr_method)
        cols_to_save = [c for c in edge_df.columns if c != "_set"]
        _save(edge_df[cols_to_save] if "_set" in edge_df.columns else edge_df,
        ...
        if G.number_of_edges() > 2:
            ...
            _save(mdf, str(self.out / "motifs.csv")); res["motifs"] = mdf
        ...
        if c.causal.enabled:
            ...
            if gene_df is not None:
                ...
                combined = pd.concat([class_df.reset_index(drop=True),
                                      gene_df.reset_index(drop=True)], axis=1)
            else:
                combined = class_df.reset_index(drop=True)
            DG, det = pc_skeleton(combined, feats, c.causal.alpha, c.causal.max_cond_set)
            _save(det, str(self.out / "causal_edges.csv")); res["causal"] = det
        ...
        if c.hypergraph.enabled:
            ...
            hedges = extract_hyperedges(class_df, c.hypergraph.min_pattern,
                                         c.hypergraph.max_pattern, c.hypergraph.min_support)
            save_cols = [c2 for c2 in hedges.columns if c2 != "_set"]
            _save(hedges[save_cols], str(self.out / "hyperedges.csv")); res["hyperedges"] = hedges
            try:
                hmdl = pattern_mdl_compression(hedges)
                if not hmdl.empty:
                    _save(hmdl[[c for c in hmdl.columns if c != '_set']], str(self.out / 'hyperedges_mdl_compression.csv')); res['hyperedges_mdl']=hmdl
            except Exception as e:
                logger.warning('Hyperedge MDL skipped: %s', e)
            hc = hypergraph_centrality(hedges, class_df.columns.tolist())
            _save(hc, str(self.out / "hypergraph_centrality.csv")); res["hg_cent"] = hc

            # Interaction information
            ii = interaction_information(class_df, class_df.columns.tolist(),
                                         c.hypergraph.max_interaction_order,
                                         c.hypergraph.min_support)
            _save(ii, str(self.out / "interaction_information.csv")); res["ii"] = ii
        ...
        try:
            ...
            if not shapley.empty:
                _save(shapley, str(self.out / 'mdr_shapley_contributions.csv')); res['shapley']=shapley
        except Exception as e:
