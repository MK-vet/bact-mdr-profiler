...
def pheno_df():
    ...
    df = pd.DataFrame({
        ...
        "AMP": rng.binomial(1, 0.6, n), "AMX": rng.binomial(1, 0.55, n),
        "TET": rng.binomial(1, 0.5, n), "DOX": rng.binomial(1, 0.45, n),
        "ERY": rng.binomial(1, 0.3, n), "AZI": rng.binomial(1, 0.25, n),
    })
...
class TestClassification:
    ...
    def test_mdr_spectrum(self, pheno_df, ontology):
        ...
        assert set(sp["Category"].unique()).issubset({"susceptible", "MDR", "XDR", "PDR"})
...
class TestHypergraph:
    ...
    def test_interaction_information_range(self, pheno_df, ontology):
        ...
        cm = build_class_matrix(pheno_df.drop(columns=["Strain_ID"]), ontology)
        ii = interaction_information(cm, cm.columns.tolist(), max_order=3,
                                     min_support=0.01)
        if not ii.empty:
            assert "II" in ii.columns
            assert set(ii["Type"].unique()).issubset({"synergistic", "redundant", "independent"})


class TestConfig:
    def test_roundtrip(self, tmp_path):
        from bactmdrprofiler.config import Config, OntologySpec
        cfg = Config(input_csv="/tmp/x.csv",
                     ontology=OntologySpec(classes={"Beta": ["AMP"]}))
        p = tmp_path / "c.yaml"
        cfg.to_yaml(p)
