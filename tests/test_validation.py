...
def coresistance_df():
    ...
    tet = np.where(amp == 1, rng.binomial(1, 0.95, n), rng.binomial(1, 0.08, n))
    df = pd.DataFrame(
        {
            "Strain_ID": [f"S{i:04d}" for i in range(n)],
            "AMP": amp,
            "AMX": rng.binomial(1, 0.5, n),
            "TET": tet,
...
class TestSignalRecovery:
    def test_pc_detects_amp_tet_edge(self, coresistance_df):
        ...
        G, _ = pc_skeleton(
            coresistance_df, ["AMP", "TET", "ERY", "GEN"], alpha=0.01, max_cond=0
        )
    ...
    def test_interaction_information_penicillin_tetracycline(
        self, coresistance_df, ontology
    ):
        ...
        ii = interaction_information(
            cm, cm.columns.tolist(), max_order=2, min_support=0.01
        )
        if not ii.empty:
            pair = ii[
                ii["Features"].apply(
            ...
            assert len(pair) > 0, (
                "Penicillins-Tetracyclines pair absent from II results"
            )
...
class TestReproducibility:
    def test_pc_skeleton_deterministic(self, coresistance_df):
        ...
        G1, _ = pc_skeleton(
            coresistance_df, ["AMP", "TET", "ERY", "GEN"], alpha=0.05, max_cond=0
        )
        G2, _ = pc_skeleton(
            coresistance_df, ["AMP", "TET", "ERY", "GEN"], alpha=0.05, max_cond=0
        )
...
class TestCLI:
    def test_help_exits_zero(self):
        r = subprocess.run(
            [sys.executable, "-m", "bactmdrprofiler.cli", "--help"], capture_output=True
        )
    ...
    def test_version_output(self):
        ...
        r = subprocess.run(
            [sys.executable, "-m", "bactmdrprofiler.cli", "--version"],
            capture_output=True,
            text=True,
        )
    ...
    def test_self_check_passes(self):
        r = subprocess.run(
            [sys.executable, "-m", "bactmdrprofiler.cli", "--self-check"],
            capture_output=True,
            text=True,
        )
        assert r.returncode == 0
